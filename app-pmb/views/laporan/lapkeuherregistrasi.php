<script>
	function pergelombang(kode){$('#fromajax').html('<img src="<?php echo base_url(); ?>/image/ajax-loader.gif" />');$.ajax ({url: "<?php echo site_url('kloader/getDataHer')?>" + "/" + kode,success: function(msg){$("#fromajax").html(msg);},error:function(){$('#fromajax').html('<center>Data yang dicari tidak ada</center>');}});}	
	$(function(){$("#datapage .paging").quickpaginate({ perpage: 30, showcounter: true, pager : $("#datapagecounter") });});	
	$('#submitcari').click(function(){var name = $('#katakunci').val();if (!name || name == 'Name') {alert('Masukkan katakunci');return false;}var form_data={katakunci: $('#katakunci').val(),ajax: '1'};$('#fromajax').html('<img src="<?php echo base_url(); ?>/image/ajax-loader.gif" />');$.ajax({url: "<?php echo site_url('kloader/getDataHer'); ?>" + "/none" ,type: 'POST',data: form_data,success: function(msg) {$('#fromajax').html(msg);},error:function(){$('#fromajax').html('<center>Data yang dicari tidak ada</center>');}});return false;});		
	$('#submitcaribytgl').click(function(){var form_data={tgl1: $('#tgl1').val(),tgl2: $('#tgl2').val(),ajax: '1'};$('#fromajax').html('<img src="<?php echo base_url(); ?>/image/ajax-loader.gif" />');$.ajax({url: "<?php echo site_url('kloader/getDataHer'); ?>" + "/none" ,type: 'POST',data: form_data,success: function(msg) {$('#fromajax').html(msg);},error:function(){$('#fromajax').html('<center>Data yang dicari tidak ada</center>');}});return false;});
